...
var likesRef=this.database.ref('likes/'+postId+'/'+this.auth.currentUser.uid);likesRef.on('value',function(data){return callback(!!data.val())});this.firebaseRefs.push(likesRef)}/**
   ...
   */},{key:'addComment',value:function addComment(postId,commentText){var commentObject={text:commentText,timestamp:Date.now(),author:{uid:this.auth.currentUser.uid,full_name:this.auth.currentUser.displayName,profile_picture:this.auth.currentUser.photoURL}};return this.database.ref('comments/'+postId).push(commentObject)}/**
...
return this.database.ref('/people/'+followedUserId+'/posts').once('value').then(function(data){var updateData={};var lastPostId=true;// Add followed user's posts to the home feed.
data.forEach(function(post){updateData['/feed/'+_this6.auth.currentUser.uid+'/'+post.key]=follow?!!follow:null;lastPostId=post.key});// Add followed user to the 'following' list.
updateData['/people/'+_this6.auth.currentUser.uid+'/following/'+followedUserId]=follow?lastPostId:null;// Add signed-in suer to the list of followers.
