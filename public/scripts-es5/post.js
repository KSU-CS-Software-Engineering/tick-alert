 ...
 */friendlyPix.Post=function(){/**
   ...
   * @constructor
   */function _class(){var _this=this;_classCallCheck(this,_class);// List of all times running on the page.
...
_this.postElement=$(friendlyPix.Post.createPostHtml());friendlyPix.MaterialUtils.upgradeTextFields(_this.postElement);_this.toast=$('.mdl-js-snackbar');_this.theatre=$('.fp-theatre')})}/**
   * Loads the given post's details.
   */_createClass(_class,[{key:'loadPost',value:function loadPost(postId){var _this2=this;// Load the posts information.
...
_this2.clear();if(!post){var data={message:'This post does not exists.',timeout:5000};_this2.toast[0].MaterialSnackbar.showSnackbar(data);if(_this2.auth.currentUser){page('/user/'+_this2.auth.currentUser.uid)}else{page('/feed')}}else{_this2.fillPostData(snapshot.key,post.thumb_url||post.url,post.text,post.author,post.timestamp,post.thumb_storage_uri,post.full_storage_uri,post.full_url)}})}/**
...
friendlyPix.firebase.cancelAllSubscriptions()}/**
   ...
   */},{key:'displayComments',value:function displayComments(comments){var commentsIds=Object.keys(comments);for(var i=commentsIds.length-1;i>=0;i--){$('.fp-comments',this.postElement).prepend(friendlyPix.Post.createCommentHtml(comments[commentsIds[i]].author,comments[commentsIds[i]].text))}}/**
   * Shows the "show more comments" button and binds it the `nextPage` callback. If `nextPage` is
   * `null` then the button is hidden.
   */},{key:'displayNextPageButton',value:function displayNextPageButton(nextPage){var _this3=this;var nextPageButton=$('.fp-morecomments',this.postElement);if(nextPage){nextPageButton.show();nextPageButton.unbind('click');nextPageButton.prop('disabled',false);nextPageButton.click(function(){return nextPage().then(function(data){nextPageButton.prop('disabled',true);_this3.displayComments(data.entries);_this3.displayNextPageButton(data.nextPage)})})}else{nextPageButton.hide()}}/**
   * Fills the post's Card with the given details.
   * Also sets all auto updates and listeners on the UI elements of the post.
   */},{key:'fillPostData',value:function fillPostData(postId,thumbUrl,imageText,author,timestamp,thumbStorageUri,picStorageUri,picUrl){var _this4=this;var post=this.postElement;// Fills element's author profile.
...
friendlyPix.firebase.registerForThumbChanges(postId,function(thumbUrl){_this4._setupThumb(thumbUrl,picUrl)});this._setupDate(postId,timestamp);this._setupDeleteButton(postId,author,picStorageUri,thumbStorageUri);this._setupLikeCountAndStatus(postId);this._setupComments(postId,author,imageText);return post}/**
   ...
   * Leaves the theatre mode.
   */},{key:'enterTheatreMode',value:function enterTheatreMode(picUrl){var _this5=this;$('.fp-fullpic',this.theatre).prop('src',picUrl);this.theatre.css('display','flex');// Leave theatre mode if click or ESC key down.
this.theatre.off('click');this.theatre.click(function(){return _this5.leaveTheatreMode()});$(document).off('keydown');$(document).keydown(function(e){if(e.which===27){_this5.leaveTheatreMode()}})}/**
   * Shows the thumbnail and sets up the click to see the full size image.
   * @private
   */},{key:'_setupThumb',value:function _setupThumb(thumbUrl,picUrl){var _this6=this;var post=this.postElement;$('.fp-image',post).css('background-image','url("'+(thumbUrl?thumbUrl.replace(/"/g,'\\"'):'')+'")');$('.fp-image',post).unbind('click');$('.fp-image',post).click(function(){return _this6.enterTheatreMode(picUrl||thumbUrl)})}/**
...
this.timers.push(setInterval(function(){return $('.fp-time',post).text(friendlyPix.Post.getTimeText(timestamp))},60000))}/**
   ...
   * @private
   */},{key:'_setupComments',value:function _setupComments(postId,author,imageText){var _this7=this;var post=this.postElement;// Creates the initial comment with the post's text.
...
$('.fp-comments',post).empty();friendlyPix.firebase.getComments(postId).then(function(data){_this7.displayComments(data.entries);_this7.displayNextPageButton(data.nextPage);// Display any new comments.
...
$('.fp-action',post).css('display','flex')}}/**
   ...
   * @private
   */},{key:'_setupDeleteButton',value:function _setupDeleteButton(postId,author,picStorageUri,thumbStorageUri){var _this8=this;var post=this.postElement;if(this.auth.currentUser&&this.auth.currentUser.uid===author.uid&&picStorageUri){$('.fp-delete-post',post).show();$('.fp-delete-post',post).off('click');$('.fp-delete-post',post).click(function(){swal({title:'Are you sure?',text:'You will not be able to recover this post!',type:'warning',showCancelButton:true,confirmButtonColor:'#DD6B55',confirmButtonText:'Yes, delete it!',closeOnConfirm:false,showLoaderOnConfirm:true,allowEscapeKey:true},function(){$('.fp-delete-post',post).prop('disabled',true);friendlyPix.firebase.deletePost(postId,picStorageUri,thumbStorageUri).then(function(){swal({title:'Deleted!',text:'Your post has been deleted.',type:'success',timer:2000});$('.fp-delete-post',post).prop('disabled',false);page('/user/'+_this8.auth.currentUser.uid)}).catch(function(error){swal.close();$('.fp-delete-post',post).prop('disabled',false);var data={message:'There was an error deleting your post: '+error,timeout:5000};_this8.toast[0].MaterialSnackbar.showSnackbar(data)})})})}else{$('.fp-delete-post',post).hide()}}/**
